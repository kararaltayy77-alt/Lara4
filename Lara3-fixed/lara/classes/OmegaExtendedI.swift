...
private func _findProcI(pid: Int32, mgr: laramgr) -> UInt64? {
    ...
    while ptr != 0 && !seen.contains(ptr) {
        seen.insert(ptr)
        let p_pid = Int32(bitPattern: ds_kread32(ptr + UInt32(off_proc_p_pid)))
        if p_pid == pid { return ptr }
        ptr = ds_kreadptr(ptr + UInt32(off_proc_p_list_le_next))
    }
    return nil
}
...
private func _taskInfo(pid: Int32, mgr: laramgr) -> String? {
    ...
    let procRo = _kreadPtrI(procPtr + UInt64(off_proc_p_proc_ro))
    let taskPtr = _kreadPtrI(procRo + UInt64(off_proc_ro_pr_task))
    guard taskPtr != 0 else { return nil }

    let vmMap = _kreadPtrI(taskPtr + UInt64(off_task_map))
    let itkSpace = _kreadPtrI(taskPtr + UInt64(off_task_itk_space))
    let threadsNext = _kreadPtrI(taskPtr + UInt64(off_task_threads_next))
    let excGuard = _kread32I(taskPtr + UInt64(off_task_task_exc_guard))
    ...
    while threadPtr != 0 && !threadSeen.contains(threadPtr) && threadCount < 1024 {
        ...
        threadCount += 1
        threadPtr = ds_kreadptr(threadPtr + UInt32(off_thread_task_threads_next))
    }
    ...
    if threadCount > 0 {
        ...
        while threadPtr != 0 && !threadSeen.contains(threadPtr) && idx < 8 {
            threadSeen.insert(threadPtr)
            let tid = _kread64I(threadPtr + UInt64(off_thread_ctid))
            let state = _kread32I(threadPtr + 0x14)
            let kstack = _kreadPtrI(threadPtr + UInt64(off_thread_machine_kstackptr))
...
private func _ucredInfo(pid: Int32, mgr: laramgr) -> String? {
    guard let procPtr = _findProcI(pid: pid, mgr: mgr) else { return nil }
    let procRo = _kreadPtrI(procPtr + UInt64(off_proc_p_proc_ro))
    let ucredPtr = _kreadPtrI(procRo + UInt64(off_proc_ro_p_ucred))
    ...
    let cr_label = _kreadPtrI(ucredPtr + UInt64(off_ucred_cr_label))

    var sandboxStr = "none"
    if cr_label != 0 {
        let sbPtr = _kreadPtrI(cr_label + UInt64(off_label_l_perpolicy_sandbox))
        if sbPtr != 0 {
    ...
    var amfiStr = "none"
    if cr_label != 0 {
        let amfiPtr = _kreadPtrI(cr_label + UInt64(off_label_l_perpolicy_amfi))
    ...
    let textvp = _kreadPtrI(procPtr + UInt64(off_proc_p_textvp))
...
private func _vmmapK(pid: Int32, mgr: laramgr) -> String? {
    ...
    let taskPtr = _kreadPtrI(procRo + UInt64(off_proc_ro_pr_task))
    guard taskPtr != 0 else { return nil }
    let vmMapPtr = _kreadPtrI(taskPtr + UInt64(off_task_map))
    guard vmMapPtr != 0 else { return nil }

    let hdrPtr = vmMapPtr + UInt64(off_vm_map_hdr)
    let nentries = _kread32I(hdrPtr + UInt64(off_vm_map_header_nentries))
    ...
    let firstEntry = _kreadPtrI(hdrPtr + UInt64(off_vm_map_header_links_next))
    ...
    while entryPtr != 0 && !entrySeen.contains(entryPtr) && count < 512 {
        ...
        let alias = _kread16I(entryPtr + UInt64(off_vm_map_entry_vme_alias))
        let objOrDelta = _kread64I(entryPtr + UInt64(off_vm_map_entry_vme_object_or_delta))
        ...
        if objOrDelta != 0 && (objOrDelta & 1) == 0 {
            let voSize = _kread64I(objOrDelta + UInt64(off_vm_object_vo_un1_vou_size))
            let voRef = _kread32I(objOrDelta + UInt64(off_vm_object_ref_count))
        ...
        entryPtr = _kreadPtrI(entryPtr + UInt64(off_vm_map_entry_links_next))
    }
    return lines.joined(separator: "\n")
}
