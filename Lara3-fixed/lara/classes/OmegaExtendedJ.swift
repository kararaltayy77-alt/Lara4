...
private func _ipcSpace(pid: Int32, mgr: laramgr) -> String? {
    ...
    let procRo = _kreadPtrJ(procPtr + UInt64(off_proc_p_proc_ro))
    let taskPtr = _kreadPtrJ(procRo + UInt64(off_proc_ro_pr_task))
    guard taskPtr != 0 else { return nil }

    let itkSpace = _kreadPtrJ(taskPtr + UInt64(off_task_itk_space))
    guard itkSpace != 0 else { return nil }

    let isTable = _kreadPtrJ(itkSpace + UInt64(off_ipc_space_is_table))
    let isTableSize = _kread32J(itkSpace + UInt64(off_ipc_space_is_table) + 8)
    ...
    for i in 0..<min(Int(isTableSize), 256) {
        ...
        let ieObject = _kreadPtrJ(entryAddr + UInt64(off_ipc_entry_ie_object))
        if ieObject == 0 { continue }

        let ipKobject = _kreadPtrJ(ieObject + UInt64(off_ipc_port_ip_kobject))
...
private func _portInfo(portAddr: UInt64) -> String? {
    ...
    let ipKobject = _kreadPtrJ(portAddr + UInt64(off_ipc_port_ip_kobject))
